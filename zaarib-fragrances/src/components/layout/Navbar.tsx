    ...
    <>
      ...
      >
        <div className="container-brand relative flex items-center justify-between h-[72px]">
          ...
          {/* Desktop Nav */}
          <nav className="hidden lg:flex flex-auto justify-center items-center lg:gap-4 xl:gap-7">
            {navLinks.map(({ label, href }) => (
              <Link
                key={href}
                href={href}
                className={[
                  "font-body text-[11.5px] tracking-[0.14em] uppercase transition-colors duration-200 whitespace-nowrap",
                  pathname === href
          ...
          {/* Actions */}
          <div className="flex-1 flex items-center justify-end gap-2.5 relative z-10">
            <a
              href={`tel:${site.phoneRaw}`}
              className="hidden lg:flex items-center justify-center gap-2 text-cream/70 hover:text-cream text-[11px] uppercase tracking-widest font-body transition-colors border border-gold/20 hover:border-gold/40 h-9 px-4 rounded-sm"
            >
              <Phone size={13} /> Call
            </a>
            ...
            <button
              onClick={openCart}
              className="relative flex items-center justify-center border border-gold/20 hover:border-gold/40 text-cream/70 hover:text-cream w-9 h-9 rounded-sm transition-all"
            ...
            <Link href="/contact" className="hidden lg:flex btn-gold h-9 !py-0 px-6 text-[11px]">
              Visit Store
            </Link>
        ...
        <div className="fixed inset-0 z-40 bg-ink/97 backdrop-blur-xl flex flex-col p-8 pt-24">
          {navLinks.map(({ label, href }) => (
            <Link
              ...
              href={href}
              className={[
                "font-display text-3xl font-normal py-5 border-b border-gold/8 transition-colors",
